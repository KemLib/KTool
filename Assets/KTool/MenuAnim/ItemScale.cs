...
namespace KTool.MenuAnim
{
    public class ItemScale : Item
    {
        ...
        public override ItemType Type => ItemType.Scale;
        ...
        protected override Tween CreateAnim(UpdateType updateType, bool unscaleTime)
        {
            ...
            switch (rtfType)
            {
                case RtfType.XY:
                    if (useOrigin)
                        rtfObject.localScale = origin;
                    tween = rtfObject.DOScale(taget, duration);
                    break;
                case RtfType.X:
                    if (useOrigin)
                        rtfObject.localScale = new Vector2(origin.x, rtfObject.anchoredPosition.y);
                    tween = rtfObject.DOScaleX(taget.x, duration);
                    break;
                case RtfType.Y:
                    if (useOrigin)
                        rtfObject.localScale = new Vector2(rtfObject.anchoredPosition.x, origin.y);
                    tween = rtfObject.DOScaleY(taget.y, duration);
                    break;
                default:
                    return null;
            }
